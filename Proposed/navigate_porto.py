import numpy as np
import copy
from eval_porto import eval_generation_agg
import random
...
def experiment(
    ...
    from get_porto_dataset import get_rl_data
    trajectories, test_trajectories, grid = get_rl_data(usr=variant['usr'])
    if variant['usr'] == 'all':
        env = portoEnv(state_dim=7, grid=grid)
    else:
        env = portoEnv(state_dim=6, grid=grid)
    max_ep_len = 256
    env_targets = [100, 50]
    scale = 10.
    ...
    for iter in range(variant['max_iters']):
        ...
        if (iter ) % 1 == 0:
            ...
            if '1' in phase:
                ...
                torch.save(trainer.model.state_dict(), f"./saved_models/{model_name}_{phase}_{iter}.pt")
            trainer.model.eval()
