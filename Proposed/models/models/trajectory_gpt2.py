...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False, is_final=False, is_estimation=False):
        ...
        if not self.is_cross_attention:
            ...
            try:
                ...
                print(mask.shape)
                print(w.shape)
                print(self.masked_bias.shape)
        # # w_ = w.cpu().detach().numpy()[0, 0]
        attention_score = torch.zeros_like(w)
    ...
    def forward(
        ...
        attn_outputs, attention_score = self._attn(query, key, value, attention_mask, head_mask, output_attentions,
                                                   is_final=is_final)
        a = attn_outputs[0]
        ...
        return outputs, attention_score  # a, present, (attentions)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        attn_outputs, attention_score = self.attn(
        ...
        return outputs, attention_score  # hidden_states, present, (attentions, cross_attentions)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        all_cross_attentions = attention_score
