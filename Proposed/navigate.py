...
def experiment(
    ...
    print(f'link_to_id: {len(link_to_id)}')
    action_counts = {}
    ...
    if model_type == "my":
        model_name = 'My_Transformer_od_awareness' + phase
    ...
    if flag == 1:
        saved_model_name = "toyota_pretrained"
    ...
    for iter in range(variant['max_iters']):
        ...
        if (iter + 0) % 20 == 0:
            if '1' in phase:
                ...
                torch.save(trainer.model.state_dict(), f"./saved_models/{model_name}_0_{phase}_{iter}.pt")
            trainer.model.eval()
            ...
            eval_generation_agg(model=trainer.model, trajectories=test_trajectories, link_to_id=link_to_id,
                                ...
                                state_std=state_std, env=env, scale=scale, model_name=saved_model_name + f"_{iter}")
            print()
    eval_generation_agg(model=trainer.model, trajectories=test_trajectories, link_to_id=link_to_id,
                        state_dim=state_dim, act_dim=act_dim, device=device, state_mean=state_mean,
                        state_std=state_std, env=env, scale=scale, model_name=saved_model_name + f"_last")
