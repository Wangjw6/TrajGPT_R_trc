...
def link_diversity(p1, p2):
    jaccard_index = []
    for i in range(min(len(p1), len(p2))):
        set1 = set(p1[i])
        set2 = set(p2[i])
        jaccard_index.append(len(set1 & set2) / len(set1 | set2))
    diversity = 1 - np.mean(jaccard_index)
    return diversity
...
def eval_generation_agg(model, trajectories, link_to_id, state_dim, act_dim, device, state_mean, state_std, env, scale,
    ...
    for i in range(len(trajectories)):
        ...
        if len(real_trajectories) <= 100000000000:
            idx.append(i)
    ...
    for i in idx:
        ...
        for j in range(len(real_trajectories[i])):
            ...
            # gen_trajectory.append(int(observation[0][1]*2) + int(observation[0][2]))
            gen_trajectory.append(int(observation[0][0]))
            notdone_obslen = torch.LongTensor([j + 1])
            ...
            if model is None:
                ...
                action = model.get_action(states,
                                          # (states - torch.from_numpy(state_mean).to(device=device)) / torch.from_numpy(state_std).to(
                                          #     device=device),
                                          actions,
                                          rewards,
                                          target_return,
                ...
                # calculate entropy of the action array
    ...
    Jaccard = get_similarity(gen_trajectories, real_trajectories, jaccard_similarity, "Jaccard")
    Cosine = get_similarity(gen_trajectories, real_trajectories, cosine_similarity, "Cosine")
    BLEU = get_similarity(gen_trajectories, od_trajectories, bleu_score, "BLEU")
