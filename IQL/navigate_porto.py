...
from IQL.models.iq_model_porto import *
...
def experiment(variant):
    ...
    if variant['usr'] == 'all':
        env = portoEnv(grid=grid, state_dim=7)
    else:
        env = portoEnv(grid=grid, state_dim=6)
    ...
    for e in range(opt['epochs']):
        ...
        if (e) % 100 == 0:
            # save model
            if e >= 0:
                torch.save(agent.q_net.state_dict(), f'./save_preference/iql_preference_global_porto{e}.pth')
                agent.q_net.eval()
                eval_generation_agg(model=agent, trajectories=test_trajectories, link_to_id=grid,
                                state_dim=state_dim, act_dim=act_dim, device=device, state_mean=1,
                                state_std=1, env=env, scale=1, model_name="IQL")
