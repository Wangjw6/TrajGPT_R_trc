...
class Critic(nn.Module):
    ...
    def embed(self, x, traj=None):
        o, d, link, speed, t, depart = x[:, 0], x[:, 1], x[:, 2], x[:, 3], x[:, 4], x[:, 6]
        usr_id = x[:, 5]
        ...
        state_embeddings = state_embeddings + embed_timestep
...
class SoftQ(object):
    def __init__(self, opt):
        ...
        self.critic_optimizer = Adam(self.q_net.parameters(), lr=opt["lr"], weight_decay=1e-5)
        self.train()
