...
class Critic(nn.Module):
    def __init__(self, device, obs_size=7, action_dim=9, seq_len=256, h_dim=256, is_traj=False, num_pref=6):
        ...
        self.h_dim = h_dim
        self.embed_link = nn.Embedding(16384, h_dim)
        self.embed_o = nn.Embedding(16384, h_dim)
        self.embed_d = nn.Embedding(16384, h_dim)
        # self.embed_lat = nn.Embedding(256, h_dim)
        # self.embed_lon = nn.Embedding(256, h_dim)
...
class SoftQ(object):
    ...
    def iq_update_critic(self, expert_batch, od_expert_batch=None):
        ...
        # add entropy term
        entropy = Categorical(logits=q+q_).entropy().mean()
