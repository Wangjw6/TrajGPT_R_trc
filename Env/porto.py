...
with open(f"./offline_data_hub/opendata/porto_action_dict.pkl", 'rb') as file:
    porto_action_dict = pickle.load(file)

with open(f"./offline_data_hub/opendata/porto_inverse_action_dict.pkl", 'rb') as file:
    porto_inverse_action_dict = pickle.load(file)


class portoEnv(gym.Env):
    def __init__(self, train_data=None, test_data=None, grid=None, state_dim=6):
        ...
        self.inverse_action_dict = porto_inverse_action_dict
        self.action_dict = porto_action_dict
        self.grid = grid
    ...
    def step(self, obs, action):
        ...
        lon_shift = int(self.inverse_action_dict[action].split('_')[1])
        lat_shift = int(self.inverse_action_dict[action].split('_')[0])
        ...
        fix_part = obs[3:].tolist()
        # print(next_lat, next_lon, self.tdrive_inverse_action_dict[action].split('_'))
        try:
            if (next_lat, next_lon) not in self.grid:
                next_obs = np.array([current_grid, current_lat, current_lon] + fix_part)
                r = 0.
                flag = 1
        ...
        next_obs = np.array([next_grid, next_lat, next_lon] + fix_part)
        # if self.state_dim == 7:
