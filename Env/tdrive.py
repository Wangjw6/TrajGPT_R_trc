...
with open(f"./offline_data_hub/opendata/tdrive_action_dict.pkl", 'rb') as file:
    tdrive_action_dict = pickle.load(file)

with open(f"./offline_data_hub/opendata/tdrive_inverse_action_dict.pkl", 'rb') as file:
    tdrive_inverse_action_dict = pickle.load(file)



class TdriveEnv(gym.Env):
    def __init__(self, train_data=None, test_data=None, grid=None, state_dim=6):
        ...
        self.tdrive_inverse_action_dict = tdrive_inverse_action_dict
        self.tdrive_action_dict = tdrive_action_dict
        self.grid = grid
    ...
    def step(self, obs, action):
        ...
        try:
            if (next_lat, next_lon) not in self.grid or next_lat < 0 or next_lat > 256 or next_lon < 0 or next_lon > 256:
