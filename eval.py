...
def calculate_self_bleu(trajectories):
    scores = []
    for i in range(len(trajectories)):
        candidate = trajectories[i]
        references = [traj for j, traj in enumerate(trajectories) if i != j]
        # Calculate BLEU score treating one trajectory as candidate and all others as references
        score = bleu.sentence_bleu(references, candidate, weights=(0.25, 0.25, 0.25, 0.25))
        scores.append(score)
    # Average BLEU score across all trajectories
...
def eval_generation_agg(model, trajectories, link_to_id, state_dim, act_dim, device, state_mean, state_std, env, scale,
    ...
    for i in range(len(trajectories)):
        ...
        for j in range(trajectories[i]['observations'].shape[0]):
            real_trajectory.append(trajectories[i]['observations'][j][2])
    ...
    for i in idx:
        ...
        for j in range(trajectories[i]['observations'].shape[0] + 10):
            ...
            if action not in action_record:
